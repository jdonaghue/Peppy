...
test("element", function() {
	...
	ok( peppy.query("*").length >= 30, "Select all" );
	var all = peppy.query("*"), good = true;
	...
	equals( peppy.query("param", document.getElementById("object1")).length, 2, "Object/param as context" );

	isSet( peppy.query("div p"), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	isSet( peppy.query("div p"), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	isSet( peppy.query("div p"), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	isSet( peppy.query("div p"), q("firstp","ap","sndp","en","sap","first"), "Finding elements with a context." );
	
	ok( peppy.query("#length").length, '&lt;input name="length"&gt; cannot be found under IE, see #945' );
	ok( peppy.query("#lengthtest input").length, '&lt;input name="length"&gt; cannot be found under IE, see #945' );

	// Check for unique-ness and sort order
	isSet( peppy.query("*"), peppy.query("*, *"), "Check for duplicates: *, *" );
	isSet( peppy.query("p"), peppy.query("p, div p"), "Check for duplicates: p, div p" );
...
test("id", function() {
	...
	equals( peppy.query("#tName1")[0].id, 'tName1', "ID selector with same value for a name attribute" );
	equals( peppy.query("#tName2").length, 0, "ID selector non-existing but name attribute on an A tag" );
	t( "ID Selector on Form with an input that has a name of 'id'", "#lengthtest", ["lengthtest"] );
	
	t( "ID selector with non-existant ancestor", "#asdfasdf #foobar", [] ); // bug #986

	isSet( peppy.query("body div#form"), [], "ID selector within the context of another element" );
...
test("class", function() {
	...
	isSet( peppy.query("p .blog"), q("mark", "simon"), "Finding elements with a context." );
	isSet( peppy.query("p .blog"), q("mark", "simon"), "Finding elements with a context." );
	isSet( peppy.query("p .blog"), q("mark", "simon"), "Finding elements with a context." );
	isSet( peppy.query("p .blog"), q("mark", "simon"), "Finding elements with a context." );
	...
	isSet( peppy.query(".e", div), [ div.firstChild ], "Finding a second class." );

	div.lastChild.className = "e";

	isSet( peppy.query(".e", div), [ div.firstChild, div.lastChild ], "Finding a modified class." );
});
...
test("name", function() {
	...
	isSet( peppy.query("#form input[name=action]"), q("text1"), "Name selector within the context of another element" );
	isSet( peppy.query("#form input[name='foo[bar]']"), q("hidden2"), "Name selector for grouped form element within the context of another element" );
...
test("child and adjacent", function() {
	...
	isSet( peppy.query("* > :first-child", document.getElementById("nothiddendiv")), q("nothiddendivchild"), "Verify child context positional selector" );
	isSet( peppy.query("* > :nth-child(1)", document.getElementById("nothiddendiv")), q("nothiddendivchild"), "Verify child context positional selector" );
	isSet( peppy.query("* > *:first-child", document.getElementById("nothiddendiv")), q("nothiddendivchild"), "Verify child context positional selector" );
	...
	divs = peppy.query("p:first-child", null, null);
...
test("attributes", function() {
	...
	peppy.query("form input")[0].test = 0;
	peppy.query("form input")[1].test = 1;
...
test("pseudo (:) selectors", function() {
	...
	var div = peppy.query('#nothiddendivchild')[0];
